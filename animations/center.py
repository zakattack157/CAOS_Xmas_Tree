...
expand_speed = 5.0   # How fast the "sphere" expands per cycle (units in coordinate space)
...
def getNextColor():
    ...
    r, g, b = palette[current_color_index]
    current_color_index = (current_color_index + 1) % len(palette)
    return [int(r * max_brightness), int(g * max_brightness), int(b * max_brightness)]
...
# MAIN EFFECT
def xmaslight(): # NOTE: change to (duration=None):
    
    # NOTE:
    # ADD THIS: start_time = time.time()
...
xmaslight() # remove this and replace with below
#
# NOTE:
#
